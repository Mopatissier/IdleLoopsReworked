Views.registerView("timeControls", {
    ...
    // todo: add talent tree
        ...
        <div class='button showthatO control' onclick='toggleOffline()'>${_text("time_controls>bonus_seconds>title")}
            <div class='showthis' style='width:230px;color:black;'>${_text("time_controls>bonus_seconds>main_text")}
                <div class='bold' id='isBonusOn'>${_text("time_controls>bonus_seconds>state>off")}</div><br>
				<div class='bold'>${_text("time_controls>bonus_seconds>bonus_mult")}</div> <div id='bonusMult'></div>
                <div class='bold'>${_text("time_controls>bonus_seconds>counter_text")}</div> <div id='bonusSeconds'></div>
            </div>
        </div>
        <div id='talentTreeBtn' style='display: none;' onclick='view.showTalents()'' class='button control'>${_text("time_controls>talents_button")}</div>
